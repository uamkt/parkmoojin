...
export async function commitArticleToGitHub(
  ...
  const baseMessage = `feat(cbrn-en): ${article.title} [auto]\n\nGenerated by Vercel Cron CBRN Publisher.\nDate: ${article.date}\nSlug: ${article.slug}`;
  ...
  // 기존 파일 존재 여부 확인 (재시도/같은 날 재실행 대비)
  let existingSha: string | undefined = undefined;
  try {
    const checkUrl = `${apiUrl}?ref=${encodeURIComponent(branch)}`;
  ...
  const message = existingSha
    ? `${baseMessage}\n\n[update existing file — same-day retry]`
    : baseMessage;
